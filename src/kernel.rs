...
impl Kernel {
    ...
    fn klog(&mut self, subsys: &str, msg: &str) {
        let ts = self.ticks as f64 * 0.000001;
        self.log.push(format!("[{:12.6}] {}: {}", ts, subsys, msg));
    }
    pub fn generate_boot_log(&mut self) {
        ...
        self.klog("bios", "POST");
        self.ticks += 1;
        self.klog("bios", &format!("mem {}K OK", self.mem.total / 1024));
        self.ticks += 1;
        self.klog("bios", "boot device: wasm0");
        self.ticks += 10;
        self.state = KernelState::Boot;
        self.klog("boot", &format!("kpawnd kernel {}", VERSION));
        self.ticks += 1;
        self.klog("boot", "loading kernel...");
        self.ticks += 100;
        self.state = KernelState::Init;
        self.klog("kernel", "starting");
        self.ticks += 1;
        let _ = self.mem.alloc(4096);
        self.klog(
            "mm",
            &format!("page allocator init, {} pages free", self.mem.free / 4096),
        );
        self.ticks += 1;
        self.fs.init();
        self.klog("vfs", "rootfs mounted");
        self.ticks += 1;
        self.klog("vfs", "devfs mounted on /dev");
        self.ticks += 1;
        self.klog("vfs", "procfs mounted on /proc");
        self.ticks += 1;
        self.klog("tty", "tty0: 80x25");
        self.ticks += 1;
        ...
        self.klog("kernel", "runlevel 3");
        self.ticks += 1;
        self.klog("kernel", "BOOT_COMPLETE");
    }
