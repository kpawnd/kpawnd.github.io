...
impl PythonInterpreter {
    ...
    pub fn eval(&mut self, code: &str) -> Result<String, String> {
        ...
        if code.contains("import") || code.contains("__") || code.contains("eval") 
            || code.contains("exec") || code.contains("open") || code.contains("file")
        ...
        if let Some(eq_pos) = trimmed.find('=') {
            if !trimmed[..eq_pos].contains('>') && !trimmed[..eq_pos].contains('<') 
                && !trimmed[..eq_pos].contains('!') && !trimmed[..eq_pos].contains('=') {
    ...
    fn eval_expression(&self, expr: &str) -> Result<PythonValue, String> {
        ...
        if expr.starts_with("len(") && expr.ends_with(")") {
            ...
            match val {
                ...
                PythonValue::String(s) => s.parse::<i64>()
                    .map(PythonValue::Int)
            ...
            match val {
                ...
                PythonValue::String(s) => s.parse::<f64>()
            ...
            if let Some(pos) = expr.rfind(*op) {
                if pos > 0 && pos < expr.len() - 1 {
                    ...
                    return match (left, right) {
                        (PythonValue::Int(a), PythonValue::Int(b)) => {
                            match op {
